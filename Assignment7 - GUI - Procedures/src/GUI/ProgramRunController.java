...
public class ProgramRunController implements Initializable {
    ...
    @FXML
    private TableView<Map.Entry<String, Pair<List<String>,IStmt>>> procTableView=new TableView<>();

    @FXML
    private TableColumn<Map.Entry<String, Pair<List<String>,IStmt>>, Pair<String,List<String>>> procNameTableColumn=new TableColumn<>();

    @FXML
    private TableColumn<Map.Entry<String, Pair<List<String>,IStmt>>, String> procBodyTableColumn=new TableColumn<>();
...
//String, Pair<List<String>,IStmt>
    ...
    private void loadData(){
        ...
        if(selectedProgram!=null){
            ...
            try {
                IDict<String, IValue> symbolTable = this.selectedProgram.getSymTable().clone().pop();
            ...
            IProcedureTable procTable=selectedProgram.getProcTable();
            List<Map.Entry<String, Pair<List<String>,IStmt>>> procTableList=new ArrayList<>();
            for(Map.Entry<String, Pair<List<String>,IStmt>> element:procTable.getProcTable().entrySet()){
                Map.Entry<String, Pair<List<String>,IStmt>> el=new AbstractMap.SimpleEntry<String, Pair<List<String>,IStmt>>(element.getKey(),element.getValue());
                procTableList.add(el);
            }
            procTableView.setItems(FXCollections.observableList(procTableList));
            procTableView.refresh();

            procNameTableColumn.setCellValueFactory(p->new SimpleObjectProperty(new Pair<String,List<String>>(p.getValue().getKey(),p.getValue().getValue().getKey())));
